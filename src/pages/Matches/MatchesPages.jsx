        ...
        <div className="page fade-in">
            ...
            <div className="topbar sticky" style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                    <span className="topbar-title font-oswald" style={{ letterSpacing: '0.05em', fontSize: 22 }}>Умные совпадения</span>
                    <div style={{ width: 44, height: 44, borderRadius: 14, background: 'var(--primary-light)', color: 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontFamily: "'Oswald', sans-serif" }}>
                        {matches.length}
                    </div>
                </div>
            </div>
            ...
            <div className="page-content" style={{ padding: '12px 20px 120px', gap: 16 }}>
                {matches.length === 0 ? (
                    <div className="empty-state" style={{ padding: '60px 0' }}>
                        ...
                        <div className="font-oswald" style={{ fontSize: 20, fontWeight: 700, textTransform: 'uppercase', marginBottom: 8 }}>Совпадений нет</div>
                        <div style={{ color: 'var(--text-muted)', marginBottom: 24 }}>Добавьте больше объектов или уточните запросы</div>
                    </div>
                ) : (
        ...
        <div className="card card-clickable" onClick={onClick} style={{ 
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ 
                        ...
                        fontWeight: 800, fontFamily: "'Oswald', sans-serif", fontSize: 16
                    }}>
                        {m.score}%
                    </div>
                    <div className="font-oswald" style={{ fontWeight: 700, fontSize: 14, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text)' }}>
                        {lvl.label}
                    </div>
                </div>
                <div style={{ 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ padding: '12px 16px', background: 'var(--bg-light)', borderRadius: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 2 }}>Объект</div>
                        <div style={{ fontWeight: 700, fontSize: 14 }}>{prop ? `${formatPrice(prop.price)} · ${prop.address || prop.city}` : 'Объект удалён'}</div>
                    </div>
                </div>
                ...
                <div style={{ padding: '12px 16px', background: 'var(--bg-light)', borderRadius: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 2 }}>Клиент</div>
                        <div style={{ fontWeight: 700, fontSize: 14 }}>{buyer?.full_name || 'Контакт скрыт'} · до {formatPrice(req?.budget_max)}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    if (!match) return (
        <div className="page">
            <div className="topbar">
                ...
                <span className="topbar-title font-oswald">Не найдено</span>
            </div>
        </div>
    );
    ...
    return (
        <div className="page fade-in" style={{ paddingBottom: 120 }}>
            <div className="topbar" style={{ background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(20px) saturate(180%)', padding: '20px', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
                ...
                <span className="topbar-title font-oswald" style={{ letterSpacing: '0.05em' }}>Анализ совпадения</span>
                <div style={{ 
                    ...
                    fontWeight: 800, fontFamily: "'Oswald', sans-serif"
                }}>
                    {match.score}%
                </div>
            </div>
            ...
            <div className="page-content" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div className="card" style={{ padding: '28px', border: 'none', boxShadow: '0 8px 32px rgba(0,0,0,0.03)', borderRadius: 32, background: 'white' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <span className="font-oswald" style={{ fontWeight: 800, fontSize: 18, textTransform: 'uppercase' }}>{lvl.label}</span>
                        <div style={{ 
                            padding: '4px 10px', borderRadius: 8, fontSize: 10, fontWeight: 800, textTransform: 'uppercase',
                    ...
                    <div style={{ textAlign: 'right', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)' }}>{match.score} / 100</div>
                </div>
                ...
                {prop && (
                    <div className="card card-clickable" onClick={() => navigate(`/properties/${prop.id}`)} style={{ padding: '24px', border: 'none', boxShadow: '0 8px 32px rgba(0,0,0,0.03)', borderRadius: 32, background: 'white' }}>
                        <div className="font-oswald" style={{ fontWeight: 700, color: 'var(--primary)', fontSize: 11, marginBottom: 8, textTransform: 'uppercase' }}>Объект в продаже</div>
                        <div className="font-oswald" style={{ fontWeight: 800, fontSize: 20, marginBottom: 4 }}>{formatPrice(prop.price)}</div>
                        <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>{prop.rooms > 0 ? `${prop.rooms}-комнатная` : 'Студия'} · {prop.area_total}м² · {prop.floor}/{prop.floors_total} эт.</div>
                        <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{prop.address}</div>
                    </div>
                )}
                ...
                {buyer && (
                    <div className="card card-clickable" onClick={() => navigate(`/clients/${buyer.id}`)} style={{ padding: '24px', border: 'none', boxShadow: '0 8px 32px rgba(0,0,0,0.03)', borderRadius: 32, background: 'white' }}>
                        <div className="font-oswald" style={{ fontWeight: 700, color: '#f59e0b', fontSize: 11, marginBottom: 8, textTransform: 'uppercase' }}>Запрос на покупку</div>
                        <div className="font-oswald" style={{ fontWeight: 800, fontSize: 20, marginBottom: 4 }}>{buyer.full_name}</div>
                        {req && (
                            <>
                                <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>Бюджет: {req.budget_min ? `${formatPrice(req.budget_min)} — ` : ''}{formatPrice(req.budget_max)}</div>
                                <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{req.rooms?.map(r => r === 0 ? 'студия' : `${r}к`).join('/')} · {req.city}</div>
                            </>
                        )}
                    </div>
                )}
                ...
                <div className="card" style={{ padding: '24px', border: 'none', boxShadow: '0 8px 32px rgba(0,0,0,0.03)', borderRadius: 32, background: 'white' }}>
                    <div className="font-oswald" style={{ fontWeight: 700, fontSize: 14, textTransform: 'uppercase', marginBottom: 12, color: 'var(--text-muted)' }}>Заметки риелтора</div>
                    <textarea className="form-textarea" style={{ minHeight: 80, borderRadius: 16, background: 'var(--bg-light)', border: 'none', padding: 12, fontWeight: 600 }} value={comment} onChange={e => setComment(e.target.value)} placeholder="Заметки по этому совпадению..." />
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 12 }}>
                        ...
                        {saved && <span style={{ color: 'var(--success)', fontSize: 12, fontWeight: 700 }}>✓ Сохранено</span>}
                    </div>
                </div>
                ...
                {showShowingForm && (
                    <div className="card fade-in" style={{ padding: '24px', border: 'none', boxShadow: '0 8px 32px rgba(0,0,0,0.1)', borderRadius: 32, background: 'white' }}>
                        <div className="font-oswald" style={{ fontWeight: 700, marginBottom: 12, textTransform: 'uppercase' }}>Назначить показ</div>
                ...
                {match.status !== 'rejected' && match.status !== 'deal' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 8 }}>
                        {!showShowingForm && (
                            <button className="btn btn-primary btn-full" style={{ height: 56, borderRadius: 18, fontSize: 15, fontWeight: 800, textTransform: 'uppercase' }} onClick={() => setShowShowingForm(true)}>Назначить показ</button>
                        )}
                        ...
                        {buyer && (
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                                ...
                                }}><Phone size={18} /> <span style={{ fontSize: 13, fontWeight: 800, textTransform: 'uppercase' }}>Звонок</span></a>
                                
                                <button className="card-clickable" style={{ 
                                    height: 56, borderRadius: 18, background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, color: 'var(--primary)', border: '1px solid var(--border-light)'
                                }} onClick={() => setIsMsgModalOpen(true)}><Send size={18} /> <span style={{ fontSize: 13, fontWeight: 800, textTransform: 'uppercase' }}>Шаблон</span></button>
                            </div>
                        )}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                            <button className="card-clickable" style={{ 
                                ...
                                <Share2 size={18} /> <span style={{ fontSize: 13, fontWeight: 800, textTransform: 'uppercase' }}>Ссылка</span>
                            </button>
                            <button className="card-clickable" style={{ 
                                ...
                                <Trash size={18} /> <span style={{ fontSize: 13, fontWeight: 800, textTransform: 'uppercase' }}>Отказ</span>
                            </button>
                        </div>

                        {match.status === 'showing_done' && (
                            <button className="btn btn-success btn-full" style={{ height: 56, borderRadius: 18, background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: 'white', fontWeight: 800, textTransform: 'uppercase' }} onClick={handleDeal}>Оформить сделку</button>
                        )}
                    </div>
                )}
            </div>
