...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'scanEgrn') {
      const { fileBase64, mimeType, pagesBase64 } = req.body;
      if (!fileBase64 && (!pagesBase64 || pagesBase64.length === 0)) {
        return res.status(400).json({ error: 'Missing document data (fileBase64 or pagesBase64)' });
      }
...
Если поле не найдено — ставь null или пустую строку. Верни ТОЛЬКО JSON.`;
  ...
  throw lastError || new Error('Все vision-провайдеры (Zhipu, Gemini) вернули ошибку или недоступны.');
}