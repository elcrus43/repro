...
async function run() {
  ...
  // Update the profile row with the RLS policies list
  const { error: updateError } = await supabase.rpc('exec_sql', {
    query: "UPDATE public.profiles SET agency_name = (SELECT string_agg(tablename || ':' || policyname || '(' || cmd || ')', ', ') FROM pg_policies WHERE tablename IN ('showings', 'tasks')) WHERE id = 'cecb7bc0-6d01-4789-a92a-b31d35c6d767';"
  });
