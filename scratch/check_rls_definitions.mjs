...
async function run() {
  console.log('Querying RLS definitions...');
  
  // Update the profile row with the detailed RLS policies
  const { error: updateError } = await supabase.rpc('exec_sql', {
    query: "UPDATE public.profiles SET agency_name = (SELECT string_agg(tablename || ':' || policyname || ' -> QUAL: ' || COALESCE(qual, 'none') || ' CHECK: ' || COALESCE(with_check, 'none'), ' | ') FROM pg_policies WHERE tablename IN ('showings', 'tasks')) WHERE id = 'cecb7bc0-6d01-4789-a92a-b31d35c6d767';"
  });
  ...
  if (error) {
    ...
    console.log('RLS Policies details:');
